  ...
  render() {
    ...
    if (!(deployment)) {
      return (<div className="container-fluid">
          <Helmet title="Manage - Auth | Hasura" />
          <h2> Manage Auth </h2>
          <hr/>
      ...
      <div className="container-fluid">
        ...
        <div className="row">
          <div className="col-xs-12">
            <p>
              <b>External endpoint:</b> <code>{'https://' + `auth${domain}/`}</code>
            </p>
          </div>
        </div>
        <div className="row">
          <div className="col-xs-12">
            <p>
              <b>Internal endpoint:</b> <code>{'http://' + `auth.${globals.namespace}/`}</code>
            </p>
          </div>
        </div>
        <hr/>
...
AuthManage.propTypes = {
  deployment: PropTypes.object,
  dispatch: PropTypes.func.isRequired
};

const mapStateToProps = (state) => {
  return {...state.authManage};
};

export default connect(mapStateToProps)(AuthManage);
